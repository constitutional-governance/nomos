Feature: Kafka topic naming convention
  ...
  Scenario: Complete valid topic name passes
    Given the topic name "raw.retail.pos.acme.orders.receipt.v1"
    When I validate it
    Then validation passes

  @enforced
  Scenario: kstreams internal topic passes
    Given the topic name "kstreams.retail.pos.acme.receipt.dlq.v1"
    When I validate it
    Then validation passes

  @enforced
  Scenario: landing topic for a connector source passes
    Given the topic name "landing.retail.pos.acme.lookup.product.v1"
    When I validate it
    Then validation passes

  @enforced
  Scenario: Fewer than seven segments is rejected
    Given the topic name "raw.retail.pos.acme.v1"
    When I validate it
  ...
  Scenario: Unknown prefix is rejected
    Given the topic name "internal.retail.pos.acme.orders.receipt.v1"
    When I validate it
    Then validation fails
  ...
  Scenario: Missing version suffix is rejected
    Given the topic name "raw.retail.pos.acme.orders.receipt.latest"
    When I validate it
    Then validation fails
  ...
  Scenario: Hyphens inside a segment are rejected
    Given the topic name "raw.retail.pos.acme.orders.guest-receipt.v1"
    When I validate it
    Then validation fails
  ...
  Scenario: dev prefix passes but warns about production use
    Given the topic name "dev.retail.pos.acme.orders.receipt.v1"
    When I validate it
    Then validation passes
